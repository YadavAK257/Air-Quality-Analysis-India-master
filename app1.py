...
with st.sidebar:
    choose = option_menu("Air Quality Index",
                         ["About", "Data Analysis", "Data Visualization", "Prediction", "Conclusion"],
...
prediction_data['AQI'] = prediction_data.apply(lambda x: cal_aqi(x['SOi'], x['NOi'], x['RSPMi'], x['SPMi'], x['PMi']),
                                               axis=1)
...
if choose == "About":
    ...
    * **Pollutants(Features) that we are going to analyze and use for prediction:** 
    ...
    st.write(
        'AQI can be divided into few category based on their AQI values. Each category has its own related to its impact on the envionment.')
